...
const requestParkingInfo = async function (
  data,
  vehicleTypes,
  carparkIds,
  lang
  ...
  let endpoint = `${url}${!!data ? `data=${data}` : 'info'}${
    !!vehicleTypes ? `&vehicleTypes=${vehicleTypes}` : ''
  }${!!carparkIds ? `&carparkIds=${carparkIds}` : ''}${
    !!lang ? `&lang=${lang}` : ''
  }
    `;
  const response = await fetch(endpoint);
...
const getParkingInfo = function (response) {
  ...
  for (let i = 0; i < response.length; i++) {
    ...
    info['heightLimits'] =
      !!cur.heightLimits && !!cur.heightLimits[0]
        ? cur.heightLimits[0].height
        : undefined;
...
const updateModal = async function (index) {
  ...
  const address = !!parkingLot.address
    ...
    <p>
        Building Name: ${
          parkingLot.address.buildingName
            ? parkingLot.address.buildingName
            : 'N/A'
        }
        <br>
        Street Name: ${
          parkingLot.address.streetName ? parkingLot.address.streetName : 'N/A'
        },
        <br>
        Building Number: ${
          parkingLot.address.buildingNo ? parkingLot.address.buildingNo : 'N/A'
        }
        <br>
        Floor: ${parkingLot.address.floor ? parkingLot.address.floor : 'N/A'}
        <br>
        Sub District: ${
          parkingLot.address.subDistrict
            ? parkingLot.address.subDistrict
            : 'N/A'
        },
        <br>
        District: ${
          parkingLot.address.dcDistrict ? parkingLot.address.dcDistrict : 'N/A'
        }
        <br>
  ...
  contactNo.innerHTML = `
    <p>
        Opening hours (weekdays): ${
          !!parkingLot.openingHours && !!parkingLot.openingHours.weekdays
            ? formatArray(parkingLot.openingHours.weekdays)
            : 'N/A'
        }
    </p>
    <hr>
    <div class="d-flex flex-row justify-content-between">
        <div>Start hour: ${
          !!parkingLot.openingHours
            ? parkingLot.openingHours.periodStart
            : 'N/A'
        }</div>
        
        <div>End hour: ${
          !!parkingLot.openingHours ? parkingLot.openingHours.periodEnd : 'N/A'
        }</div>
        <br>
    </div>
    <hr>
    <p>
        Height Limit: ${
          !!parkingLot.heightLimits ? parkingLot.heightLimits + ' m' : 'N/A'
        } 
    </p>
    <hr>
    <p>
    Website: ${
      !!parkingLot.website
        ? `<a href=${parkingLot.website}}>${parkingLot.website}</a>`
        : 'N/A'
    }
    </p>
    <hr>
    ...
    <p>
        Payment method(s): ${
          !!parkingLot.paymentMethods
            ? formatArray(parkingLot.paymentMethods)
            : 'N/A'
        }
    </p>
    <hr>
  ...
  mapButton.addEventListener('click', (e) => {
    window.open(
      `https://www.google.com/maps/@${parkingLot.latitude},${parkingLot.longitude},15z`
    );
  });
};
...
const displayPagination = function (totalParkingLots) {
  ...
  for (
    let i = 0;
    i < Math.ceil(totalParkingLots / parkingLotPerPage) && i < maxPage;
    i++
  ) {
...
const displayParkingLotCards = function (parkingLots, page) {
  ...
  for (
    let i = page * parkingLotPerPage;
    i < page * parkingLotPerPage + parkingLotPerPage;
    i++
  ) {
...
const onFiltered = function (e) {
  const value =
    selectVehicleTypes.value === 'all' ? undefined : selectVehicleTypes.value;
...
const search = function (searchValue) {
  ...
  for (let i = 0; i < parkingLotInfo.length; i++) {
    ...
    const fullAddress = !!parkingLotInfo[i].address
      ? getFullAddress(parkingLotInfo[i].address)
      : getFullAddress({});
    if (
      name.includes(searchValue.toLowerCase()) ||
      fullAddress.includes(searchValue.toLowerCase())
    ) {
